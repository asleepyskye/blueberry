...
const updatePinnedMessage = async (ctx: Context) => {
  ...
  let newMessage = await ctx.rest.createMessage(
    config.update_requests_channel,
    pinnedMessageText,
  );
  if (currentMessageId)
    ctx.rest.deleteMessage(config.update_requests_channel, currentMessageId);
  await ctx.db.level.put(key, newMessage.id);
};

const lockUnlockChannel = async (
  ctx: Context,
  guildId: string,
  channelId: string,
  lock: boolean,
) => {
  let flags = await ctx.rest
    .fetchChannel(channelId)
    .then(
      (x: any) =>
        x.permission_overwrites.find((o: any) => o.id == guildId).deny,
    );
  let operator = lock ? " | " : " & ~";
...
export default async (evt: any, ctx: Context) => {
  ...
  if (evt.channel_id == config.update_requests_channel)
    await updatePinnedMessage(ctx);
  ...
  if (content == "?rank chat access") {
    ...
    if (
      evt.member.roles.includes(config.restrict_role_id) ||
      ![
    ...
    if (evt.member.roles.includes(config.chat_role_id)) {
      let roles = evt.member.roles.filter(
        (role: string) => role != config.chat_role_id,
      );
      ...
      await ctx.rest.editGuildMember(evt.guild_id, evt.author.id, {
  ...
  if (content?.startsWith("?incident ")) {
    ...
    if (subcommand == "active") {
      ...
      subcommand.startsWith("create") &&
      evt.member.roles.includes(config.staff_role_id)
    ) {
      /*
				formatting:
      ...
      subcommand.startsWith("editupdate") &&
      evt.member.roles.includes(config.staff_role_id)
    ) {
      /*
				formatting:
      ...
      subcommand.startsWith("addupdate") &&
      evt.member.roles.includes(config.staff_role_id)
    ) {
      /*
				formatting:
  ...
  if (
    ...
    evt.member.roles.includes(config.admin_role_id)
  ) {
    let res;
    try {
  ...
  if (
    ...
    evt.member.roles.includes(config.infra_role_id)
  ) {
    if (content == "+s h" || content == "+s help") {
      ...
      console.log(res);
      let silences = res
        .map((x: any) => `- \`${x.check ?? x.checkPrefix + "*"}@${x.node}\``)
        .join("\n");
      ...
      if (newsilence.length != 2) {
        await ctx.rest.createMessage(
          evt.channel_id,
          "format: `check@host`, optionally accepting a wildcard for hostname (`somecheck@*`) and/or at end of check name (someprefix*@somehost)",
        );
        return;
      }
      // parse text
      newsilence = [newsilence]
        .map((x) => ({ check: x[0], checkPrefix: null, node: x[1] }))
        .map((x) => {
          if (x.check.endsWith("*")) {
      ...
      if (
        !!curSilences.find(
          (x: any) =>
            x.check == newsilence.check &&
            x.checkPrefix == newsilence.checkPrefix &&
            x.node == newsilence.node,
        )
      ) {
