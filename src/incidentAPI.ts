...
export async function getIncident(id: string): Promise<Incident> {
    ...
    return {
        ...
        resolution_timestamp: data.resolution_timestamp ? new Date(data.resolution_timestamp) : null,
        updates: data.updates ? data.updates.map((update: any) => ({
            ...update,
            timestamp: new Date(update.timestamp),
        })) : [],
    } as Incident;
 ...
 */
export async function createUpdate(incidentID: string, text: string): Promise<string> {
    const response = await fetch(`${baseURL}/api/v1/admin/incidents/${incidentID}/update`, {
        method: "POST",
        body: text
    });
    const data = await response.text();
